...
# STUDY TWO, Change Voter Identification and Attribute Independence Testing --------------------------
...
here::i_am("code/2_ANES_identification_theoryforming.R")
options(scipen = 999) # we hate scientific notation of decimals
...
# load our large list of relevant variables to be tested
...
# 1.2 Vote-Pattern Filtering (Obama->Trump->Biden) --------------------------------------------------
# voted_change_20_allelecs_slct 
#Create CVoters: Voted for Change in 2020, 16, 12
...
# create usable dataset of our CVoters
...
# create usable dataset of our MVoters
...
# 2.1 COMPARISON - Manual View Summary Stats-------------------------------------------------------------------------

# not the best. tough to eyeball things. 
...
# 2.3 COMPARISON - Statistical Independence Testing -------------------------------------------------------------------------

### 2.3.1 T-Test on all variables -----------------------------------------------------------------
...
### 2.3.1 Ordinal - Wilcoxon Rank-Sum Test --------------------------------------------------
...
### Joining together all statistical test results -------------------------------------------------------
...
### Filtering - only p-value < 0.05 -------------------------------------------------------

# p-value < 0.05
joint_comparison_significant <- joint_comparison %>%
  filter(t_pval <= 0.05 | w_pval <= 0.05 | f_pval <= 0.05) %>% # any of the tests
  arrange(t_pval) %>%
  left_join(var_attributes_2020, by = "variable") %>%
  select(variable, 
...
### Plot - all stat. independent vars ------------------------------------------------------------------
# Get all variable names
...
# Combine all plots into 5 rows x 4 columns (we got 20 variables)
ggsave(here("outputs/ANES_attributes_independent.png"), 
