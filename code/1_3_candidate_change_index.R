...
# creaing Candidate Change Index
# combining candidate posturing (rhetoric) and perception (attributes) 
...
here::i_am("code/1_3_candidate_change_index.R")
...
# ref dataset for CCI
ref_cci <- read_csv(here("data/ref/candidate_attributes.csv"))
# includes things like age of candidates, their highest govt office, gender...

crosstab_campaigndocs_totals <- read_csv(here("data/results/crosstab_campaigndocs_totals.csv"))
# results from our dictionary analysis
...
# 1. Join together crosstab totals and candidate attributes  -------------------------------------------------------
...
cci_attributes <- ref_cci %>% 
  ...
  # alternative age-scoring formulas:
  ...
  # 'within year relative changegrams'
  group_by(year) %>%
  mutate(max_year_changegram = max(percent_changegrams, na.rm = TRUE)) %>%
  mutate(relative_inyear_changegrams = percent_changegrams / max_year_changegram) %>% # this is what we use for CCI
  ...
  # for fun: relative to all nominees changegrams
  group_by(nominee_yn) %>%
...
# 2. Set Weights and Create CCI -------------------------------------------------------------------
# CCI (Candidate Change Index) Weights

# weighting scheme: 50% posturing, 50% attributes, even weighting across attributes
weight_changegrams <- 0.5 # posturing (change vocab frequency)
...
cci_attributes_weighted <- cci_attributes_clean %>% # mutate new column that hold weighted scores
...
cci_attributes_weighted %>%
  write_csv(here("data/results/candidates/CCI_results.csv"))
