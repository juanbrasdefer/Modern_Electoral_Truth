...
score_changevoter <- function(dataset, score_economic = "score_economic", 
  ...
  dataset_scored <- dataset %>%
  mutate(changevoter_score = ((0.50* !!sym(score_economic)) + (0.35 * !!sym(score_leftright)) + (0.15 * !!sym(score_informed))))
...
filter_ANES_attributes_OR <- function(dataset, var_to_find_1, var_to_find_2, year_of_dset, ref_df = var_names_ref_discrete){
